...
puppet_config_set() {
  "${PUPPET_BIN_DIR?}/puppet" config set server puppetmaster.sxkoxi1m2bqujhxq4i00de5lbb.dx.internal.cloudapp.net --section main
...
supported_platform() {
  ...
  t_supported_platform_regex='(el-(5|6|7|8)-(i386|x86_64|s390x|ppc64le|aarch64))|(redhatfips-7-x86_64)|(debian-(8|9)-(i386|amd64))|(ubuntu-(14\.04|16\.04)-(i386|amd64|ppc64el))|(ubuntu-18\.04-amd64)|(sles-(11|12|15)-(i386|x86_64|s390x|ppc64le))|(fedora-(26|27|28|29)-(x86_64))|(solaris-(10|11)-(i386|sparc))|(aix-(6\.1)-power)|(osx-10\.(12|13|14)-x86_64)'
...
run_agent_install_from_url() {
    ...
    The required agent packages should be retrieved when puppet runs on the master, after which you can run the install.bash script again."
            ...
            fail "This method of agent installation is not supported for ${PLATFORM_TAG?} in Puppet Enterprise v2019.1.0"
        fi
    fi
    ...
    # Test for Debian releases
        ...
        if grep -qi 'amazon linux' /etc/system-release; then
          ...
          sanitize_platform_release
            ...
            fail "$(cat /etc/system-release) is not a supported platform for Puppet Enterprise v2019.1.0
        ...
        fail "$(uname -s) is not a supported platform for Puppet Enterprise v2019.1.0
            Please visit http://links.puppetlabs.com/puppet_enterprise_${PE_LINK_VER?}_platform_support to request support for this platform."
    fi
fi
...
bulk_pluginsync "https://puppetmaster.sxkoxi1m2bqujhxq4i00de5lbb.dx.internal.cloudapp.net:8140/packages/bulk_pluginsync.tar.gz"

run_agent_install_from_url "https://puppetmaster.sxkoxi1m2bqujhxq4i00de5lbb.dx.internal.cloudapp.net:8140/packages/2019.1.0/${PLATFORM_TAG}.bash" "$@"
